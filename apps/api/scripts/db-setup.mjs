 ...
 *   2. CHECK constraints — prisma/migrations_manual/checks.sql (รันซ้ำได้)
 *   3. prisma/seed.ts            (แท็ก/หมวดหมู่ — upsert รันซ้ำได้)
 *   4. prisma/seed-mock-novels.ts (ข้อมูล mock — ลบชุดเดิมแล้วสร้างใหม่ทุกครั้ง)
 *   5. prisma/seed-demo-library.ts (ผู้ใช้เดโม My Library — demo_reader@demo.buddybook.local / demo1234)
 *   6. sync กราฟ Neo4j จาก Postgres ผ่าน POST /api/v1/internal/recommendations/sync
...
async function syncGraph() {
  console.log("\n6/6 sync กราฟ Neo4j จาก Postgres");
...
run("1/6 prisma migrate deploy", prismaCli, ["migrate", "deploy"]);
run("2/6 CHECK constraints (checks.sql)", prismaCli, [
  "db",
  "execute",
  "--file",
  "prisma/migrations_manual/checks.sql",
  "--schema",
  "prisma/schema.prisma",
]);
run("3/6 seed.ts (แท็ก/หมวดหมู่)", tsxCli, ["prisma/seed.ts"]);
if (args.has("--skip-mock")) console.log("\n4-5/6 ข้าม seed-mock-novels.ts + seed-demo-library.ts (--skip-mock)");
else {
  run("4/6 seed-mock-novels.ts (ข้อมูล mock)", tsxCli, ["prisma/seed-mock-novels.ts"]);
  run("5/6 seed-demo-library.ts (ผู้ใช้เดโม My Library)", tsxCli, ["prisma/seed-demo-library.ts"]);
}
if (args.has("--skip-graph")) console.log("\n6/6 ข้าม sync Neo4j (--skip-graph)");
